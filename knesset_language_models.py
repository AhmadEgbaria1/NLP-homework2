...
def complete_masked_sentence(lm, masked_sentence):
    ...
    while "[*]" in tokens:
        ...
        next_token, _ = lm.gener3ate_next_token(prefix)
...
################################### q 3.4 ###################
# standard perplexity model
def calculate_perplexity(model, sentences):
    """
    sentences: list of sentences (without starting tokens).
   calculating the preplexity o every token in every sentence.
    """
    total_log_prob = 0.0
    total_tokens = 0
    ...
    for sentence in sentences:
        ...
        for i in range(2, len(tokens)):
            ...
            if p <= 0.0:
                p = 1e-12
            # Log base 2 (log2) – acceptable for perplexity
