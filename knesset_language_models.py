...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python trigram_test.py <path/to/corpus.jsonl>")
        sys.exit(1)
    ...
    # בונים מודל עם סימני פיסוק
    print("=== בונה מודל עם סימני פיסוק... ===")
    lm_full = TrigramLM(jsonl_path, remove_punct=False)

    # בונים מודל בלי סימני פיסוק
    print("=== בונה מודל בלי סימני פיסוק... ===")
    lm_no_punct = TrigramLM(jsonl_path, remove_punct=True)

    # משפט לבדיקה – תחליף למה שבא לך
    test_sentence = "אני חושב שזה חשוב מאוד"

    print("\n--- בדיקת calculate_prob_of_sentence ---")
    logp_full = lm_full.calculate_prob_of_sentence(test_sentence)
    logp_no_punct = lm_no_punct.calculate_prob_of_sentence(test_sentence)

    print(f"משפט לבדיקה: {test_sentence}")
    print(f"log P (עם פיסוק): {logp_full}")
    print(f"log P (בלי פיסוק): {logp_no_punct}")

    print("\n--- בדיקת generate_next_token ---")
    next_tok_full, logp_next_full = lm_full.generate_next_token(test_sentence)
    next_tok_no_punct, logp_next_no_punct = lm_no_punct.generate_next_token(test_sentence)

    print(f"next token (עם פיסוק): {next_tok_full}, logP = {logp_next_full}")
    print(f"next token (בלי פיסוק): {next_tok_no_punct}, logP = {logp_next_no_punct}")
